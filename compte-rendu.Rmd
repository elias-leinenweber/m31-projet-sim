...
\begin{algorithm}[ht!]
  ...
  \Entree{$p, k, n$}
  \PourCh{slot}{
    Marquer comme non occupé \;
    (Ré)-initialiser la liste des transmetteurs \;
    \PourCh{station}{
      \Si{le slot a été choisi pour retransmettre}{
        Démarrer une tentative \;
        \SinonSi{rand}{
          $\tt{queued_msgs} \leftarrow \tt{queued_msgs} + 1$ \;
          Démarrer une tentative \;
        }
      }
    }
...
p <- ggplot(data = res, aes(x = slots, y = queued_msgs, colour = factor(np))) +
     ...
     labs(x = "Durée en slots",
          ...
          color = "n * p",
          title = "Aloha discrétisé (0.001 <= p <= 0.005, k = 4, 10 <= n <= 50)")
print(p)
```
...
On peut resserrer l'intervalle d'étude autour de $n = 0,3$, qui a l'air d'être
...
p <- ggplot(data = res, aes(x = slots, y = queued_msgs, colour = factor(np))) +
     ...
     labs(x = "Durée en slots",
          y = "Nombre moyen de messages en attente d'émission",
          color = "n * p",
          title = "Aloha discrétisé (0.00875 <= p <= 0.01, k = 4, 20 <= n <= 40)")
print(p)
```
...
p <- ggplot(data = res, aes(x = slots, y = queued_msgs, colour = factor(np))) +
     ...
     labs(x = "Durée en slots",
          y = "Nombre moyen de messages en attente d'émission",
          color = "n * p",
          title = "Aloha discrétisé avec relance \"beb\" (0.00875 <= p <= 0.01, k = 4, 30 <= n <= 40)")
print(p)
```
...
du niveau de congestion du réseau, diverge peu importe la valeur de $np$ avec
relance uniforme.
